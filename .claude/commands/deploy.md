...
2. **Ensure the contract namespace is the user's own.** If `contracts/leaderboard/Cargo.toml` still reads the shipped placeholder `@your-username/leaderboard`, the contract can't be deployed under it (`playground deploy` runs a contract pre-step). Resolve the user's CDM handle from the playground CLI — the logged-in account's handle (check `playground --help`, e.g. `playground whoami`); **if no one is logged in, ask them to log in first** (`playground init`). Then replace `your-username` with that handle across the repo: `grep -rn "@your-username/leaderboard" --include="*.ts" --include="*.toml" --include="*.json" . | grep -v node_modules`. (Skip if it's already set to a real handle.)
